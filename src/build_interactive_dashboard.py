...
<body>
    ...
    <section id="historyPage" class="page">
      <div class="grid">
        ...
        <div class="panel span-12">
          ...
          <svg id="histGasChart" class="large-chart"></svg>
        </div>
        <div class="panel span-12">
          <div class="chart-title">Brent historico</div>
          <svg id="histBrentChart" class="large-chart"></svg>
        </div>
        <div class="panel span-12">
          <div class="chart-title">USD/BRL historico</div>
          <svg id="histUsdChart" class="large-chart"></svg>
        </div>
        <div class="panel span-12">
          <div class="chart-title">Paridad ANP etanol/gasolina C</div>
          <svg id="histParityChart" class="large-chart"></svg>
  ...
  <script>
    ...
    function lineChart(svg, series, yLabel, xLabel) {
      const width = 920, height = svg.classList.contains("large-chart") ? 500 : 360, pad = { l:72, r:26, t:18, b:54 };
    ...
    function renderHistory() {
      ...
      lineChart($("histParityChart"), [
        {name:"Paridad neta", color:colors.parity, data: rows.filter(r => r.anp_parity_net!==null).map(r => ({date:r.date, value:Number(r.anp_parity_net)}))},
        {name:"Paridad bruta", color:colors.parityGross, dashed:true, data: rows.filter(r => r.anp_parity_gross!==null).map(r => ({date:r.date, value:Number(r.anp_parity_gross)}))}
